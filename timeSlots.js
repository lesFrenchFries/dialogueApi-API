...
class TimeSlots {
    ...
    getFreeSlots(input) {
        ...
        weekDates.forEach((date,day)=>{ // Day by day
            bookingLoader.getAllBookingsForDay(date)
            .then(bookings => { // Verify for each time slot
                weekAvailabilities[day]=weekAvailabilities[day].map((specialists,idx) => { // Slot by slot
